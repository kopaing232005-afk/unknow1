...
# ------------------------- CONFIG -------------------------
BOT_TOKEN = '8871277058:AAH9rrAqXxLAtlIDwblohiIa_PtCl5nAvB0'
...
def _feistel_round(left, right, key, half_bits):
    h = hashlib.blake2s(f"{right}:{key}".encode(), digest_size=4).digest()
    f = int.from_bytes(h, "big") & ((1 << half_bits) - 1)
    return right, left ^ f


def _feistel_permute(x, bits, rounds, key):
    half_bits = bits // 2
    mask = (1 << half_bits) - 1
    ...
    for r in range(rounds):
        left, right = _feistel_round(left, right, key + r, half_bits)
...
def iter_codes(mode):
    if mode in ("6", "7"):
        ...
        while produced < total:
            value = _feistel_permute(i, bits, 4, key)
            i += 1

            if value >= total:
                continue

            yield f"{value:0{length}d}"
            produced += 1
        return
